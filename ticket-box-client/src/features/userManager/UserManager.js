...
const UserManager = () => {
  ...
  const handleSelectUser = (u) => {
    setUserSelected({
      avatar:
        u.avatar.search("https:") !== -1
          ? u.avatar
          : `https://ticket-box-bs.herokuapp.com/image/${u.avatar}`,
      name: !u.name ? "" : u.name,
    ...
    <div className="user-manager-container">
      <div className="user-info-manager">
        <div className="uiam">
          ...
          />
          <div
            className="dash-isdead didfs"
            style={{ opacity: !userSelected.isAlive ? "1" : "0" }}
          ></div>
          <div
            className="dash-isdead didsc"
            style={{ opacity: !userSelected.isAlive ? "1" : "0" }}
          ></div>
        </div>
      ...
      <div className="user-list-manager">
        {listUSer.map((user) => {
          return (
            <div
              key={user._id}
              className={`user-element-manager ${handleIsAlive(user)}`}
              style={handleSelectStyle(user)}
              ...
              <img
                ...
                src={
                  user.avatar.search("https:") !== -1
                    ? user.avatar
                    : `https://ticket-box-bs.herokuapp.com/image/${user.avatar}`
                }
                alt={user.name}
              />
